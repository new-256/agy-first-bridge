...
export function apply(ctx) {
  ...
  function statusSnapshot() {
    return { state: status.state, running: status.running, lastStatus: status.lastStatus, lastAt: status.lastAt, lastConversationId: status.lastConversationId, fallbackActive: status.fallbackActive, current: status.current, trail: status.trail.slice(-MAX_TRAIL), updatedAt: status.updatedAt }
  }
  function begin() { status.running += 1; status.state = 'running'; status.updatedAt = Date.now() }
  function end(res) {
    status.running = Math.max(0, status.running - 1)
    status.lastStatus = res ? res.status : null
    status.lastAt = Date.now()
    if (res && res.conversationId) status.lastConversationId = res.conversationId
    if (res && res.fallback) { status.fallbackActive = true; status.state = 'fallback' }
    else if (status.running > 0) { status.state = 'running' }
    else { status.state = res && res.ok ? 'ok' : 'failed' }
    status.current = null
  ...
  function startLiveParser(handle) {
    let cursor = 0
    const tick = () => {
      try {
  ...
  async function coreExecute(rawArgs, exec) {
    ...
    if (args.background && jobs && exec && exec.agent) {
      try {
        ...
        end({ ok: false, status: 'JOB_START_ERROR' })
      }
    }
