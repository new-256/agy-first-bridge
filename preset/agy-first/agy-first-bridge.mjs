  ...
  async function coreExecute(rawArgs, exec) {
    ...
    try {
      ...
      if (q && q.ok) {
        ...
        const fiveHBuckets = (q.groups || []).flatMap((g) =>
          (g.buckets || [])
        ...
        const low5h = fiveHBuckets.find((b) => typeof b.remainingFraction === 'number' && b.remainingFraction < 0.10)
        if (low5h) {
          const blockRes = {
            ok: false, status: 'QUOTA_BLOCKED', response: '', conversationId: null, durationSeconds: null, numTurns: null, totalTokens: null, exitCode: null, mode: built.mode,
