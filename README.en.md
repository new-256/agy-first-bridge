...
# 3) append the host row to cordis.patch.yml (bare name, HMR auto-reloads):
#    - insert:
#        - id: agy-indicator
#          name: agy-indicator
```
...
### DSH version compatibility (quick reference)

Full evidence lives in [docs/SUPPORT.md](docs/SUPPORT.md).

- **Supported floor: `@deepseek-ai/dsh ≥ 0.1.2-rc.1`** — only from this version did `dsh-client-modules` complete `reloadUrls` / `exactPackageSpecifier` and the surrounding contract (the `arrive()` check "the `id` in `__ModuleLoader__.load({id})` must equal the package name" exists as far back as `0.0.1-rc.1`, but early versions lack the supporting contract, so they are **not** supported).
- **Current development baseline: `0.1.7-rc.1`** (the `next` dist-tag; preset mounting, light, tools and fallback all verified locally). Note `latest` is still `0.1.5-rc.3` — two release lines are live.
- **Workflow-engine rename boundary (auto-adapted since v1.7.1)**: `dsh-base@0.1.6-alpha.1` renamed `@deepseek-ai/dsh-workflow-worker-thread` to `@deepseek-ai/dsh-workflow-ptc`. The preset now declares both rows with `!!js disabled` self-probes, so either line mounts. **1.7.0 and earlier fail to mount the preset at all on ≥0.1.6-alpha.1** (`auditRows()` rejects the entire preset, not just the workflow tools).
- **Desktop shell (DSH Desktop)**: no version requirement; the shell's enterSafeMode granularity issue is unrelated to this plugin (see `docs/issues/BACKEND-ISSUES.md`).
...
├─ LICENSE
├─ .gitignore
├─ package.json                    # version metadata (v1.7.1, Node >=18)
...
| --- | --- | --- |
| [v1.7.1](https://github.com/new-256/agy-first-bridge/releases/tag/v1.7.1) | ≥ 0.1.2-rc.1 (**required for ≥0.1.6-alpha.1**; 0.1.7-rc.1 verified) | **Fix the agent preset failing to mount at all on newer DSH (upstream workflow-engine rename)**: `@deepseek-ai/dsh-workflow-worker-thread` was renamed to `@deepseek-ai/dsh-workflow-ptc` at `dsh-base@0.1.6-alpha.1`; the hard-coded old name fails to import, and `auditRows()` rejects the **entire** preset mount when any enabled row fails (it does not merely lose the workflow tools). Because `latest` (0.1.5-rc.3) still ships the old name while `next`/`alpha` (0.1.7-*) ship the new one, **both release lines are live**, so a plain rename is not an option — the preset now declares both rows and each self-disables via `!!js disabled`, probing the running CLI's own resolution base with `createRequire(process.argv[1])`. All three paths (modern / legacy / unknown tree) were verified to enable exactly one engine, and an unknown tree still mounts the preset. Adds verify §6 static gate + `tests/preset-workflow.test.mjs` semantic regression |
| [v1.7.0](https://github.com/new-256/agy-first-bridge/releases/tag/v1.7.0) | ≥ 0.1.2-rc.1 (0.1.5-rc.1 verified) | **Incentive-structure fix — the policy now actually wins model decisions**: (1) **context protocol**: the first dispatch carries a compact CONTEXT preamble (point at paths, never paste files; agy reads the repo itself) and follow-ups continue the same agy conversation via `agy_continue`; (2) **background by default**: `background` defaults to true when jobs are available, and the run → limited-dialog → retry/fallback decision loop now lives inside the job — background failures open the fallback dialog too (the old "background never prompts" design is gone) and background jobs gain the DSH hard-timeout guard; (3) **decision-point hooks**: `agy:policy` moved from order 5 to `TOOL_SUBAGENT+10` (right after the subagent guidance, before the tool catalog), `persona` injected into both tool-subagent rows, and the policy rewritten as a compact decision table (`quick` bounded to a <30s criterion, the quota pre-check contradiction removed); timeout default 300→600s and `tokens=` dropped from the rendered head (kept in the JSON result). ⚠️ Fails to mount its preset on DSH ≥0.1.6-alpha.1 — use ≥v1.7.1 |
| [v1.6.2](https://github.com/new-256/agy-first-bridge/releases/tag/v1.6.2) | ≥ 0.1.2-rc.1 | **Fix the status light stuck idle under the merged-package layout**: the Host half no longer resolves `mcp-live.json` via `import.meta.url`; it anchors through `AGY_MCP_LIVE_FILE` → `DSH_HOME` → `%APPDATA%\DSH Desktop\dsh-home` → `~/.dsh`; the verify gate gained a path regression guard |
| [v1.6.1](https://github.com/new-256/agy-first-bridge/releases/tag/v1.6.1) | ≥ 0.1.2-rc.1 | **Fix the whole plugin page crashing on fresh installs**: `client.js` now registers under the npm package name `agy-first-bridge` (1.6.0 wrote `agy-indicator`, violating the `dsh-client-modules` `arrive()` id contract); added the 22-check `scripts/verify.mjs` pre-publish gate |
