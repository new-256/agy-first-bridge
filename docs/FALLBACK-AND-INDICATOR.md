...
end(res) // running--, 据结果置 ok/failed/fallback
```

Host 通过 `harness.handle('agy_status', () => snapshot())` 暴露只读快照；Client 组件用 `ctx.interval` 每 1.2s `host.call('agy_status')` 拉取并重渲染。这是标准的 Client→Host 包私有 RPC，快照只含标量字段，不含任何 Host 活对象引用。
